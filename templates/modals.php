    ...
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" class="modal-content border-0 shadow-lg p-4">
            <div class="modal-header border-0 pb-2">
                <h5 class="fw-bold m-0 text-primary"><i class="bi bi-building-add me-2"></i>إضافة مشروع جديد</h5>
                <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <label class="form-label small fw-bold text-muted">اسم المشروع (عمارة، فيلا، مجمع...)</label>
                <input name="project_name" class="form-control form-control-lg border-0 bg-light py-3 fs-5" placeholder="مثلاً: مشروع حي الأمل السكني" required autofocus>
            </div>
            <div class="modal-footer border-0 pt-0">
                <button name="add_project" class="btn btn-primary w-100 py-3 fw-bold shadow-sm">حفظ المشروع والبدء بالعمل</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal fade" id="payM<?= $w['id'] ?>" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content border-0 shadow-lg p-4 text-center">
                ...
                <p class="text-muted small mb-4">للعامل: <?= htmlspecialchars($w['worker_name']) ?></p>
                <form method="POST">
                    <input type="hidden" name="worker_id" value="<?= $w['id'] ?>">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">المبلغ المراد صرفه</label>
                        <input type="number" name="amt" class="form-control form-control-lg text-center fw-bold border-0 bg-light py-3 fs-4" placeholder="0.00" required autofocus>
                    </div>
                    <div class="mb-4">
                        <label class="form-label small fw-bold text-muted">تاريخ الصرف</label>
                        <input type="date" name="p_date" class="form-control border-0 bg-light py-2 text-center" value="<?= date('Y-m-d') ?>">
                    </div>
                    <button name="add_pay" class="btn btn-primary w-100 py-2 fw-bold shadow-sm">تأكيد عملية الصرف</button>
                </form>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content border-0 shadow-lg p-0 overflow-hidden">
                ...
                <div class="modal-body p-0">
                    <div class="bg-primary bg-opacity-10 p-3 text-center border-bottom border-white">
                        <span class="text-muted small d-block mb-1">إجمالي المبلغ المتفق عليه</span>
                        <h4 class="fw-bold text-primary mb-0"><?= formatCurrency($w['total_amount']) ?></h4>
                    </div>
                        ...
                        ?>
                            <div class="list-group-item p-3 border-light">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <span class="text-muted small"><i class="bi bi-calendar3 me-1"></i> <?= $p['payment_date'] ?></span>
                                    <span class="fw-bold text-primary fs-6"><?= formatCurrency($p['amount']) ?></span>
                                </div>
                                ...
                                <div class="collapse mt-3" id="editPay<?= $p['id'] ?>">
                                    <form method="POST" class="bg-light p-3 rounded-3 shadow-sm border">
                                        <input type="hidden" name="pay_id" value="<?= $p['id'] ?>">
                                        <div class="row g-2">
                                            <div class="col-6">
                                                <label class="small text-muted mb-1">المبلغ</label>
                                                <input type="number" name="amt" class="form-control form-control-sm border-0 bg-white" value="<?= $p['amount'] ?>" required>
                                            </div>
                                            <div class="col-6">
                                                <label class="small text-muted mb-1">التاريخ</label>
                                                <input type="date" name="p_date" class="form-control form-control-sm border-0 bg-white" value="<?= $p['payment_date'] ?>" required>
                                            </div>
                                            <div class="col-12 mt-3">
                                                <button name="edit_pay" class="btn btn-sm btn-primary w-100 fw-bold py-2">حفظ التغييرات</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
    ...
    <div class="modal fade" id="sPay<?= $s['id'] ?>" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content border-0 shadow-lg p-4 text-center">
                ...
                <p class="text-muted small mb-4"><?= htmlspecialchars($s['supplier_name']) ?></p>
                <form method="POST">
                    <input type="hidden" name="supplier_id" value="<?= $s['id'] ?>">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">المبلغ المدفوع</label>
                        <input type="number" name="amount" class="form-control form-control-lg text-center fw-bold border-0 bg-light py-3 fs-4 text-success" placeholder="0.00" required autofocus>
                    </div>
                    <div class="mb-4">
                        <label class="form-label small fw-bold text-muted">تاريخ السداد</label>
                        <input type="date" name="p_date" class="form-control border-0 bg-light py-2 text-center" value="<?= date('Y-m-d') ?>">
                    </div>
                        ...
                        ?>
                            <div class="list-group-item d-flex justify-content-between align-items-center p-3 border-light">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="text-muted small"><?= $ph['payment_date'] ?></span>
                                </div>
